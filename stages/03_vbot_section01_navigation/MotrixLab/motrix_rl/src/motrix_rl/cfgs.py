...
class navigation:
    ...
    class VBotNavigationSection01PPOConfig(PPOCfg):
        ...
        play_num_envs: int = 1                 # 评估时并行环境数
        max_env_steps: int = 1024 * 60_000     # 最大训练步数
        check_point_interval: int = 500        # checkpoint 保存间隔

        # ===== 学习率调度 =====
        learning_rate: float = 3e-4            # 学习率
        learning_rate_scheduler_kl_threshold: float = 0.008  # KL 阈值调度

        # ===== PPO 算法核心参数 =====
        rollouts: int = 48                     # 每次更新采集的回合数
        learning_epochs: int = 6               # 每次更新的训练轮数
        mini_batches: int = 32                 # 每次 epoch 的小批量数
        discount_factor: float = 0.99          # GAE discount factor
        lambda_param: float = 0.95             # GAE lambda
        grad_norm_clip: float = 1.0            # 梯度裁剪阈值

        # ===== 损失函数系数 =====
        entropy_loss_scale: float = 0.0        # 熵奖励系数（0=不鼓励探索）
        value_loss_scale: float = 2.0          # value loss 系数
        time_limit_bootstrap: bool = True      # 是否对 time-limit done 做 bootstrap

        # ===== PPO 裁剪参数 =====
        ratio_clip: float = 0.2                # 策略比率裁剪 epsilon
        value_clip: float = 0.2                # value function 裁剪 epsilon
        clip_predicted_values: bool = True     # 是否裁剪 value 预测

        # ===== 网络结构 =====
        policy_hidden_layer_sizes: tuple[int, ...] = (512, 256, 128)  # 策略网络
        value_hidden_layer_sizes: tuple[int, ...] = (512, 256, 128)   # value 网络

    # ===== Disabled for Section01 homework: section02/section03 envs are not registered =====
